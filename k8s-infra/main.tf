...
locals {
  ...
  command_map = sensitive(substr(local.test, 0, 1) == "/" ? {
    command     = "oci ce cluster create-kubeconfig --cluster-id ${local.cluster_id} --file ../${path.module}/oci_kube_config.temp --token-version 2.0.0  --kube-endpoint PUBLIC_ENDPOINT",
    intrepreter = "bash"
    } : {
    command     = "oci ce cluster create-kubeconfig --cluster-id ${local.cluster_id} --file ../${path.module}/oci_kube_config.temp --region ap-chuncheon-1 --token-version 2.0.0  --kube-endpoint PUBLIC_ENDPOINT"
